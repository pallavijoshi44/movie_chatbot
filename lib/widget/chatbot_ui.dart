...
class _ChatBotUIState extends State<ChatBotUI> {
  ...
  @override
  void initState() {
    _getDialogFlowResponseByEvent(WELCOME_EVENT, DEFAULT_PARAMETERS_FOR_EVENT, true);
    super.initState();
  }
  @override
  ...
  void _getDialogFlowResponseByEvent(String eventName,
      dynamic parameters, bool firstTime) async {
    _textController.clear();
    setState(() {
  ...
  void _executeResponse(AIResponse response) {
    ...
    if (response != null) {
      ...
      if (ACTION_START_OVER == action) {
        _getDialogFlowResponseByEvent(START_OVER_EVENT, DEFAULT_PARAMETERS_FOR_EVENT, false);
      }
